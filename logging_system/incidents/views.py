...
def incidents(request, tag=None, system=None):
    ...
    return render(
        request, "incident/list.html", {"incidents": page, "systems": systems}
    )
...
def view(request, pk):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            AuditLog.objects.create(
                user=request.user,
                message=f"User {request.user} commented on incident '{incident}': {comment}",
            )
    ...
    return render(
        request,
        "incident/view.html",
        {"incident": incident, "comments": comments, "form": form},
    )
...
def remove(request, pk):
    ...
    AuditLog.objects.create(
        user=request.user,
        message=f"User {request.user} removed incident {delete_it} successfully.",
    )
    delete_it.delete()
...
def resolve(request, pk):
    incident = Incident.objects.get(id=pk)
    AuditLog.objects.create(
        user=request.user, message=f"User {request.user} resolved incident {incident}."
    )
...
def report(request, frame=0, tag=None):
    AuditLog.objects.create(
        user=request.user, message=f"User {request.user} created incidents report."
    )

    start = (
        timezone.now() - timedelta(weeks=9999)
        if frame == 0
    ...
    incidents = (
        Incident.objects.filter(date__gte=start)[:5000]
        if tag is None
        else Incident.objects.filter(date__gte=start, tag=tag).order_by("id")[:5000]
    )
