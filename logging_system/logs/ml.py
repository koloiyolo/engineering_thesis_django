...
def train(cl=None, vec=None):
    ...
    cl_params = (
        None
        if settings.s1_clusterer_parameters == ""
        else ast.literal_eval(settings.s1_clusterer_parameters)
    )
    vec_params = (
        None
        if settings.s1_vectorizer_parameters == ""
        else ast.literal_eval(settings.s1_vectorizer_parameters)
    )
    ...
    step1 = [("vectorizer", vectorizer), ("clusterer", clusterer)]
...
def cluster():
    ...
    cl_params = (
        None
        if settings.s2_clusterer_parameters == ""
        else ast.literal_eval(settings.s2_clusterer_parameters)
    )
    vec_params = (
        None
        if settings.s2_vectorizer_parameters == ""
        else ast.literal_eval(settings.s2_vectorizer_parameters)
    )
    labels = None
            ...
            step1 = Pipeline(
                [
                    (
                        "vectorizer",
                        get_preprocessor(
                            vec=settings.s1_vectorizer,
                            vec_params=settings.s1_vectorizer_parameters,
                        ),
                    ),
                    (
                        "clusterer",
                        get_clusterer(
                            cl=settings.s1_clusterer,
                            cl_params=settings.s1_clusterer_parameters,
                        ),
                    ),
                ]
            )
...
def get_clusterer(cl=1, cl_params=None):
    ...
    try:
        if cl == 1:
            ...
            clusterer = (
                AgglomerativeClustering(**cl_params)
                if cl_params
...
def get_preprocessor(vec=1, vec_params=None):
    ...
    try:
        if vec == 0:
            vectorizer = (
                TfidfVectorizer(**vec_params) if vec_params else TfidfVectorizer()
            )
        else:
            vectorizer = (
                CountVectorizer(**vec_params) if vec_params else CountVectorizer()
            )

    except Exception as e:
        print(f"Vectorizer hyperparameters error {e}")
        vectorizer = TfidfVectorizer() if vec == 0 else CountVectorizer()
    return vectorizer
